...
void apply::on_pushButton_clicked()
{
    ...
    if(ui->radioButton_coach->isChecked()==true){        //为教练，对教练相关表格进行操作
...
if(user.length()!=11||user_pwd.length()<=3)//判断密码是否小于3位，用户名是否为11位的电话号码
{
        QMessageBox::critical(this,"注册情况","登录账号为11位手机号，密码不能小于3位！");
        return;
    }
    if(user.length()==0)//判断申请的用户名是否为空
    {
